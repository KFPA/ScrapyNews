...
class SpiderRulesSingletion(object):
    ...
    def _parseBaserules(self,rule):
        ...
        spiderrule.url_format=urllist.get(xmlElements.get('URL_FORMAT'))
...
from scrapywork.settings import dbkeys
class DatabaseSingleton(object):
    ...
    def ensuretableexist(self):
        try:
            ...
            if not cursor.fetchone():
                ...
                #     % self.table
                sqlo='CREATE TABLE %s'%self.table
                sql='(id int(10) auto_increment primary key not null,url varchar(100),INDEX urlidx(url),source text(65535)'
                for key in dbkeys:
                    sql+=','+key +' text(65535)'
                sql+=')'
                cursor.execute(sqlo+sql)
        except Exception as e:
    ...
    def insertItem(self,item,source):
        try:
            cursor=self.database.cursor()
            sqlo='INSERT INTO {}'.format(self.table)
            sqlt=' VALUES(%s,%s'
            sql='(url,source'
            for key in dbkeys:
                sql+=','+key
            ...
            cursor.execute(sqll,tuple(items))
            self.database.commit()
        except Exception as e:
            self.database.rollback()
            logging.error('--------------insert false-------------------exception:%s', e)
...
# from scrapywork.settings import IMAGES_STORE,FILES_STORE,IMAGES_UPLOAD_PATH_FULL,IMAGES_UPLOAD_PATH_SMALL
#
